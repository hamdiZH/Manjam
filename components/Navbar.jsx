...
export default function Navbar() {
  ...
  return (
    <nav className="w-full bg-white relative">
      <div className="flex items-center justify-between h-[60px] md:h-[70px] lg:h-[80px] px-4 md:px-6 lg:px-0">
        {/* Mobile Menu Button */}
        <button
        ...
        <div className="hidden lg:flex items-center gap-3 xl:gap-5 w-auto xl:w-[336px] h-5">
          {navItems.map((item, index) => (
            ...
            >
              <button
                className={`flex items-center gap-1 font-medium text-xs xl:text-sm transition-opacity whitespace-nowrap font-(family-name:--font-space-grotesk) ${
                  isActive(item.id)
                    ? "text-[#10B981]" 
        ...
        <div className="flex items-center gap-2 md:gap-3 lg:gap-4">
          ...
          {/* Become a Client Button - Primary */}
          <button className="flex items-center justify-center w-auto h-[38px] md:h-[42px] lg:h-[46px] py-2 md:py-[14px] px-3 md:px-5 gap-2 rounded-lg border border-[#1C4A3F] bg-[radial-gradient(50%_50%_at_50%_50%,#111827_0%,#153830_100%)] shadow-[0px_2px_6px_0px_rgba(16,24,40,0.06)] text-white font-medium text-xs md:text-sm transition-all hover:opacity-90 cursor-pointer whitespace-nowrap">
      ...
      {/* Mobile Menu */}
      <div 
        className={`lg:hidden fixed inset-0 top-[60px] md:top-[70px] bg-white z-50 transition-transform duration-300 ${
      ...
      >
        <div className="flex flex-col p-4 h-full overflow-y-auto">
          ...
          {navItems.map((item) => (
            <div key={item.id} className="border-b border-[#E5E7EB]">
              <button
                ...
                className={`flex items-center justify-between w-full py-4 font-medium text-base ${
                  mobileActiveDropdown === item.id ? "text-[#10B981]" : "text-[#1C4A3F]"
                }`}
              >
                <span>{item.label}</span>
                <ChevronDown 
              ...
              <div className={`overflow-hidden transition-all duration-300 ${
                mobileActiveDropdown === item.id ? 'max-h-[500px] pb-4' : 'max-h-0'
              }`}>
                {item.id === "services" && (
          ...
          {/* Mobile Login Button */}
          <button className="md:hidden mt-4 flex items-center justify-center w-full h-[46px] py-[14px] px-5 gap-2 rounded-lg border border-[#1C4A3F] shadow-[0px_2px_6px_0px_rgba(16,24,40,0.06)] text-[#1C4A3F] font-medium text-sm transition-all hover:bg-[radial-gradient(50%_50%_at_50%_50%,#111827_0%,#153830_100%)] hover:text-white cursor-pointer">
            Log in
          </button>
        </div>
      </div>
        ...
        <div 
          className={`hidden lg:block absolute left-0 right-0 top-full bg-white shadow-lg z-50 transition-all duration-300 overflow-hidden ${
            servicesOpen ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'
          }`}
          onMouseEnter={() => setServicesOpen(true)}
          onMouseLeave={() => setServicesOpen(false)}
        >
          <div className="px-4 lg:px-[35px] xl:px-[50px] 2xl:px-[80px] py-6 lg:py-10">
            <div className="grid grid-cols-3 gap-4 lg:gap-6 xl:gap-8 max-w-[1440px] mx-auto">
              {serviceItems.map((item) => (
                <div key={item.id} className="flex flex-col gap-3 lg:gap-4">
                  {/* Image Container */}
                  <div className="relative w-full aspect-video max-w-[384px] rounded-xl overflow-hidden">
                    <Image
                      src={item.image}
                      alt={item.title}
                      fill
                    ...
                    {/* Badge */}
                    {item.badge && (
                      <div className="absolute top-2 lg:top-3 right-2 lg:right-3 flex items-center gap-1.5 px-2 lg:px-3 py-1 lg:py-1.5 bg-[#EF4444] text-white text-[10px] lg:text-xs font-medium rounded-full">
                        <span className="w-1.5 lg:w-2 h-1.5 lg:h-2 bg-white rounded-full"></span>
                        {item.badge}
                      </div>
                    )}
                  </div>
                  ...
                  {/* Title */}
                  <h3 className="text-[#111827] text-base lg:text-lg font-semibold">{item.title}</h3>

                  {/* Description */}
                  <p className="text-[#6B7280] text-xs lg:text-sm leading-relaxed">{item.description}</p>

                  {/* Learn More Button */}
                  <button className="w-fit flex items-center gap-2 px-3 lg:px-5 py-2 lg:py-2.5 bg-[#E6F7F1] text-[#10B981] font-medium text-xs lg:text-sm rounded-lg hover:bg-[#D1FAE5] transition-colors cursor-pointer">
                    Learn More
                    <ArrowRightIcon />
                  </button>
                </div>
        ...
        <div 
          className={`hidden lg:block absolute left-0 right-0 top-full bg-white shadow-lg z-50 transition-all duration-300 overflow-hidden ${
            insightsOpen ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'
          }`}
          onMouseEnter={() => setInsightsOpen(true)}
          onMouseLeave={() => setInsightsOpen(false)}
        >
          <div className="px-4 lg:px-[35px] xl:px-[50px] 2xl:px-[80px] py-6 lg:py-10">
            <div className="grid grid-cols-3 gap-6 lg:gap-8 xl:gap-12 max-w-[1440px] mx-auto">
              {/* Left Column - Image Card */}
              <div className="flex flex-col gap-3 lg:gap-4 justify-between">
                {/* Image Container */}
                <div className="relative w-full aspect-video max-w-[384px] rounded-xl overflow-hidden">
                  <Image
                ...
                {/* Learn More Button */}
                <button className="w-fit flex items-center gap-2 text-[#10B981] font-medium text-base lg:text-lg hover:opacity-80 transition-opacity cursor-pointer">
                  Learn More
                  <ArrowRightIcon />
                </button>
              ...
              {/* Middle Column - Articles, News, Newsletter */}
              <div className="flex flex-col gap-4 lg:gap-6 justify-between">
                {insightsItems.map((item) => (
                  <div key={item.id} className="flex flex-col gap-1 lg:gap-2">
                    {item.href ? (
                      <Link href={item.href} className="text-[#10B981] text-lg lg:text-xl font-semibold italic cursor-pointer hover:underline">
                        {item.title}
                      </Link>
                    ) : (
                      <h3 className="text-[#10B981] text-lg lg:text-xl font-semibold italic cursor-pointer">{item.title}</h3>
                    )}
                    <p className="text-[#6B7280] text-xs lg:text-sm leading-relaxed">{item.description}</p>
                  </div>
                ))}
              ...
              {/* Right Column - Learn More Links */}
              <div className="flex flex-col gap-4 lg:gap-6 justify-between">
                {insightsLearnMore.map((item) => (
                  <div key={item.id} className="flex flex-col gap-1 lg:gap-2">
                    <h3 className="text-[#10B981] text-lg lg:text-xl font-semibold cursor-pointer">{item.title}</h3>
                    <p className="text-[#6B7280] text-xs lg:text-sm leading-relaxed">{item.description}</p>
                  </div>
                ))}
              </div>
        ...
        {/* Explore Manjam Dropdown (Desktop only) */}
        <div 
          className={`hidden lg:block absolute left-0 right-0 top-full bg-white shadow-lg z-50 transition-all duration-300 overflow-hidden ${
            exploreOpen ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'
          }`}
        ...
        >
          <div className="px-4 lg:px-[35px] xl:px-[50px] 2xl:px-[80px] py-6 lg:py-10">
            <div className="grid grid-cols-3 gap-6 lg:gap-8 xl:gap-12 max-w-[1440px] mx-auto">
              ...
              {/* Middle Column - Why Manjam, About, Gallery */}
              <div className="flex flex-col gap-4 lg:gap-6 justify-between">
                {exploreItemsLeft.map((item) => (
                  <div key={item.id} className="flex flex-col gap-1 lg:gap-2">
                    {item.href ? (
                      <Link href={item.href} className="text-[#10B981] text-lg lg:text-xl font-semibold italic cursor-pointer hover:opacity-80 transition-opacity">
                        {item.title}
                      </Link>
                    ) : (
                      <h3 className="text-[#10B981] text-lg lg:text-xl font-semibold italic cursor-pointer">{item.title}</h3>
              ...
              {/* Right Column - Team, Partners, Learn More */}
              <div className="flex flex-col gap-4 lg:gap-6 justify-between">
                {exploreItemsRight.map((item) => (
                  <div 
                    key={item.id} 
                    className="flex flex-col gap-1 lg:gap-2"
                    onClick={item.id === "team" ? handleTeamClick : undefined}
                  >
                    <h3 className="text-[#10B981] text-lg lg:text-xl font-semibold cursor-pointer hover:opacity-80 transition-opacity">{item.title}</h3>
                    <p className="text-[#6B7280] text-xs lg:text-sm leading-relaxed">{item.description}</p>
                  </div>
                ))}
              </div>
